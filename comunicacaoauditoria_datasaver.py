...
def db_saver_comunicacaoauditoria():
    ...
    query = "SELECT id, situacao, estado, atividade, titulo, idtarefaassociada, titulotarefaassociada, dtprevisaoinicio, dtprevisaofim, dtrealizadainicio, dtrealizadafim, prioridade, assunto, idatividade, descricaoatividade, idsituacao, dataultimamodificacao, autorultimamodificacao, indminutadestinatario, detalhamento, anexosgerais, destinatarios, copiacomunicacao, prazo, dataenviocomunicacao, dataciencia, indminutaremetente, tarefasprecedentes, unidadesenvolvidas, coordenadorequipe, equipegeral, supervisores, estadosituacao, arquivocomportamentoespecifico, tags, pendencias, abasatividade FROM comunicacao_auditoria_auxiliar"
    # Criar uma conexão para inserir os dados no destino
    cur = conn2.cursor()
    # Executar a consulta e inserir os dados no destino
    with conn1.cursor() as cursor:
        print("Executando consulta para extarir dados da tabela de origem.")
        cursor.execute(query)
        for row in cursor:
            print(f"Inserindo/Atualizando dados: {row}")
            cur.execute(""" 
                        INSERT INTO comunicacao_auditoria (id, situacao, estado, atividade, titulo, idtarefaassociada, titulotarefaassociada, dtprevisaoinicio, dtprevisaofim, dtrealizadainicio, dtrealizadafim, prioridade, assunto, idatividade, descricaoatividade, idsituacao, dataultimamodificacao, autorultimamodificacao, indminutadestinatario, detalhamento, anexosgerais, destinatarios, copiacomunicacao, prazo, dataenviocomunicacao, dataciencia, indminutaremetente, tarefasprecedentes, unidadesenvolvidas, coordenadorequipe, equipegeral, supervisores, estadosituacao, arquivocomportamentoespecifico, tags, pendencias, abasatividade) 
                ...
                SET 
                    ...
                    indminutadestinatario = EXCLUDED.indminutadestinatario,
                    detalhamento = EXCLUDED.detalhamento,
                    anexosgerais = EXCLUDED.anexgerais,
                    destinatarios = EXCLUDED.destinatario,
                    copiacomunicacao = EXCLUDED.copiacomunicacao,
                    prazo = EXCLUDED.prazo,
                    dataenviocomunicacao = EXCLUDED.dataenviocomunicacao,
                    dataciencia = EXCLUDED.dataciencia,
                    indminutaremetente = EXCLUDED.indminutaremente,
    ...
    conn2.close()
    conn1.close()
    print(f"Inserção/Atualização de dados no banco de dados {geral_db.database2} na tabela comunicacao_auditoria finalizada com sucesso!")
    print("---------------------------------------------------------------------------------------------------------------------------------------")
    
db_saver_comunicacaoauditoria()
