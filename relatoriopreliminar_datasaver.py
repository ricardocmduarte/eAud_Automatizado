...
def db_saver_relatoriopreliminar():
    ...
    query = "SELECT id, situacao, estado, atividade, titulo, idtarefaassociada, titulotarefaassociada, dtprevisaoinicio, dtprevisaofim, dtrealizadainicio, dtrealizadafim, prioridade, assunto, idatividade, descricaoatividade, idsituacao, dataultimamodificacao, autorultimamodificacao, relatoriofinal, relatoriopreliminar, hipoteselegal, coordenadorequipe, supervisores, parecer, anexorelatorio, relatorioword, unidadesenvolvidas, relatoriocom, observadores, certificados, equipegeral, arquivocomportamentoespecifico, estadosituacao, tags, pendencias, abasatividade FROM relatorio_preliminar_auxiliar"
    ...
    conn2.close()
    conn1.close()
    print(f"Inserção/Atualização de dados no banco de dados {geral_db.database2} na tabela relatorio_preliminar finalizada com sucesso!")
    print("--------------------------------------------------------------------------------------------------------------------------------------")
    
db_saver_relatoriopreliminar()
