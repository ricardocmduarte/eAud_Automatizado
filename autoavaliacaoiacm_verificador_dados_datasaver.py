...
def autoavaliacao_iacm_verificador_dados():
    ...
    #Verificar IDs na tabela tarefas_id vinculados à atividade 'Autoavaliação IA-CM' que não estão na tabela auto_avaliacao_iacm
    cur.execute("""
        SELECT a.id, a.atividade
        FROM tarefas_id a
        LEFT JOIN auto_avaliacao_iacm b
        ON a.id = b.id AND a.atividade = b.atividade
        WHERE a.atividade = 'Autoavaliação IA-CM' AND b.id IS NULL
    """)

    result = cur.fetchall()

    if result:
        print("\nIDs na tabela tarefas_id vinculados com a atividade 'Autoavaliação IA-CM' que não existem na tabela auto_avaliacao_iacm: ")

        for row in result:
            print(row)
        exportar_dados_para_csv(result)
    else:
        print("Não existem IDs na tabela tarefas_id vinculados com a atividade 'Autoavaliação IA-CM' que não existam na tabela auto_avaliacao_iacm!")

    # Fechar conexão com o banco de dados    
    conn.close()
autoavaliacao_iacm_verificador_dados()