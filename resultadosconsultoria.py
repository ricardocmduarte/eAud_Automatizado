...
def get_resultados_consultoria(ids_resultados):

    lista_dados = []
    lista_final = []
    try:
        if ids_resultados:
            for i, id in enumerate(ids_resultados):
...
def salvar_dados(lista_resultado):
    try:
        banco = db.db_connection
        cur = banco.cursor()

        lista_resultado = db.current_datetime_query(
            lista_resultado)

        for tarefa in lista_resultado:
            lista = [
                (tarefa['id'],
                 tarefa['situacao'],
                 tarefa['estado'],
