...
def get_projeto_geral(ids_projeto):

    lista_dados = []
    lista_final = []
    try:
        if ids_projeto:
            for i, id in enumerate(ids_projeto):
...
def salvar_dados(lista_projeto):
    try:
        banco = db.db_connection
        cur = banco.cursor()

        lista_projeto = db.current_datetime_query(lista_projeto)

        for tarefa in lista_projeto:
            lista = [
                (tarefa['id'],
                 tarefa['situacao'],
                 tarefa['estado'],
