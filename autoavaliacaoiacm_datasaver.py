...
def db_saver_autoavaliacaoiacm():
    ...
    with conn1.cursor() as cursor:
        ...
        for row in cursor:
            ...
            cur.execute(""" 
                        INSERT INTO auto_avaliacao_iacm (id, situacao, estado, atividade, titulo, idtarefaassociada, titulotarefaassociada, dtprevisaoinicio, dtprevisaofim, dtrealizadainicio, dtrealizadafim, prioridade, assunto, idatividade, descricaoatividade, idsituacao, dataultimamodificacao, autorultimamodificacao, anexosgerais, equipevalidacaoexterna, unidadevalidadoras, relatoriovalidacao, equipeiacm, unidadeauditoriasuptec, tarefaprecedentes, estadosituacao, niveliacm, textohistorico, iacmplanoacao, unidadesuperior, mesconclusaoprevisto, arquivocomportamentoespecifico, textoajuda, tags, pendencias, abasatividade) 
                ...
                SET
                    ...
                    equipevalidacaoexterna = EXCLUDED.equipevalidacaoexterna,
                    unidadevalidadoras = EXCLUDED.unidadevalidadoras,
                    relatoriovalidacao = EXCLUDED.relatoriovalidacao,
                    equipeiacm = EXCLUDED.equipeiacm,
                    unidadeauditoriasuptec = EXCLUDED.unidadeauditoriasuptec,
                    tarefaprecedentes = EXCLUDED.tarefaprecedentes,
                    niveliacm = EXCLUDED.niveliacm,
                    textohistorico = EXCLUDED.textohistorico,
                    iacmplanoacao = EXCLUDED.iacmplanoacao,
                    unidadesuperior = EXCLUDED.unidadesuperior,
                    mesconclusaoprevisto = EXCLUDED.mesconclusaoprevisto,
                    textoajuda = EXCLUDED.textoajuda,
    ...
    conn2.close()     
    conn1.close()
    print(f"Inserção/Atualização de dados no banco de dados {geral_db.database2} na tabela auto_avaliacao_iacm finalizada com sucesso!")
    print("-------------------------------------------------------------------------------------------------------------------------------------")
    
db_saver_autoavaliacaoiacm()