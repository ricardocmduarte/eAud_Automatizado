...
def get_termo_compromisso_consultoria(ids_termo):

    lista_dados = []
    lista_final = []
    try:
        if ids_termo:
            for i, id in enumerate(ids_termo):
...
def salvar_dados(lista_termo):
    try:
        banco = db.db_connection
        cur = banco.cursor()

        lista_termo = db.current_datetime_query(
            lista_termo)

        for tarefa in lista_termo:
            lista = [
                (tarefa['id'],
                 tarefa['situacao'],
                 tarefa['estado'],
