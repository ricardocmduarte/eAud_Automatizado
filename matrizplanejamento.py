...
def get_matriz_planejamento(ids_matriz):

    lista_dados = []
    lista_final = []
    try:
        if ids_matriz:
            for i, id in enumerate(ids_matriz):
...
def salvar_dados(lista_matriz):
    try:
        banco = db.db_connection
        cur = banco.cursor()

        lista_matriz = db.current_datetime_query(lista_matriz)

        for tarefa in lista_matriz:
            lista = [
                (tarefa['id'],
                 tarefa['situacao'],
                 tarefa['estado'],
