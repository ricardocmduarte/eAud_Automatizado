...
def exportar_dados_para_csv(result):
    ...
    if result:
        ...
        # Nome do Arquivo csv
        arquivo = f"C://Users//M1503249//Documents//eAud_Arquivos//resultados_consultoria_auxiliar.csv"
...
def resultados_consultoria_verificador_dados():
    ...
    cur.execute("""
        ...
        FROM tarefas_id a
        LEFT JOIN resultados_consultoria b
        ON a.id = b.id AND a.atividade = b.atividade
        WHERE a.atividade = 'CGE - Resultados (Consultoria)' AND b.id IS NULL
    """)

    result = cur.fetchall()

    if result:
        print("\nIDs na tabela tarefas_id vinculados com a atividade 'CGE - Resultados (Consultoria)' que não existem na tabela resultados_consultoria: ")

        for row in result:
            print(row)
        exportar_dados_para_csv(result)
    else:
        print("Não existem IDs na tabela tarefas_id vinculados com a atividade 'CGE - Resultados (Consultoria)' que não existam na tabela resultados_consultoria!")
    
    # Fechar conexão com o banco de dados
    conn.close()

resultados_consultoria_verificador_dados()